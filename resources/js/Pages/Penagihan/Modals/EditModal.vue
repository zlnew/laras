...
import { DetailPenagihan, DetailRAB } from "@/types";
...
const form = useForm<{
  id_detail_rab: number | null;
  volume_penagihan: number;
  harga_satuan: number;
}>({
  id_detail_rab: props.detail_penagihan.id_detail_rab,
  volume_penagihan: props.detail_penagihan.volume_penagihan,
  harga_satuan: props.detail_penagihan.harga_satuan
});
...
function submit() {
  form.patch(route('penagihan.update', props.detail_penagihan.id_detail_penagihan), {
    onSuccess: () => {
      modal.close();
    }
  });
...
<template>
  <form @submit.prevent="submit">
    <modal-layout size="2xl">
      <modal-head title="Form Ubah Uraian Penagihan" />
